...
public class AddNewMemberCardBatchInfoActivity
    ...
    public void onRespStatus(String body, int source) {
        ...
        switch (NetRespStatType.dealWithRespStat(body)) {
            case STATUS_AUTHORIZE_FAIL:
                ViewHandler.exitAcitivityDueToAuthorizeFail(AddNewMemberCardBatchInfoActivity.this);
                setResult(Ref.RESULTCODE_DONE);
                finish();
                break;
            case NSR:
                ViewHandler.toastShow(AddNewMemberCardBatchInfoActivity.this,"无此记录，请重试");
                break;
            case PARTYLY_FAIL:
                ViewHandler.toastShow(AddNewMemberCardBatchInfoActivity.this,"部分会员卡办理失败");
                setResult(Ref.RESULTCODE_DONE);finish();
                break;
            case SUCCESS:
                ViewHandler.toastShow(AddNewMemberCardBatchInfoActivity.this,"会员卡办理成功");
                setResult(Ref.RESULTCODE_DONE);finish();
                break;
            case FAIL:
                ViewHandler.toastShow(AddNewMemberCardBatchInfoActivity.this,"会员卡办理失败");
                setResult(Ref.RESULTCODE_DONE);finish();
                break;
            default:break;
        }
    ...
    public void onReqFailure(Object object, int source) {
        ...
        ViewHandler.dealWithReqFail(AddNewMemberCardBatchInfoActivity.this);
    }

    @Override
    public void onRespError(int source) {
        super.onRespError(source);
        ViewHandler.toastShow(AddNewMemberCardBatchInfoActivity.this,Ref.CANT_CONNECT_INTERNET);
    }
